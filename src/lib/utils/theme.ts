...
export function parseVscodeTheme(themeJsonStr: string, name: string): ThemeConfig | null {
  const cleanJson = themeJsonStr.replace(
    /\\"|"(?:\\"|[^"])*"|(\/\/.*|\/\*[\s\S]*?\*\/)/g,
    (m, g) => (g ? "" : m),
  );
  try {
    ...
    cssVars["--color-canvas-default"] =
      colors["editor.background"] ||
      colors["window.background"] ||
      colors["sideBar.background"] ||
      (isDark ? "#1e1e1e" : "#ffffff");
    cssVars["--color-canvas-subtle"] =
      colors["sideBar.background"] ||
      colors["editorWidget.background"] ||
      (isDark ? "#252526" : "#f3f3f3");
    cssVars["--color-canvas-overlay"] =
      colors["editorWidget.background"] ||
      colors["dropdown.background"] ||
      cssVars["--color-canvas-subtle"];
    cssVars["--tab-active-bg"] =
      colors["tab.activeBackground"] ||
      colors["editorGroupHeader.tabsBackground"] ||
      cssVars["--color-canvas-default"];

    cssVars["--color-fg-default"] =
      colors["editor.foreground"] ||
      colors["sideBar.foreground"] ||
      colors["foreground"] ||
      (isDark ? "#d4d4d4" : "#333333");
    cssVars["--color-fg-muted"] =
      colors["descriptionForeground"] ||
      colors["tab.inactiveForeground"] ||
      (isDark ? "#999999" : "#666666");
    cssVars["--color-fg-subtle"] =
      colors["disabledForeground"] || cssVars["--color-fg-muted"];

    cssVars["--color-border-default"] =
      colors["activityBar.border"] ||
      colors["editorGroup.border"] ||
      colors["focusBorder"] ||
      (isDark ? "#444444" : "#dddddd");
    cssVars["--color-border-muted"] =
      colors["sideBarSectionHeader.border"] ||
      colors["widget.border"] ||
      cssVars["--color-border-default"];
    cssVars["--color-window-border-top"] =
      colors["titleBar.activeBackground"] || cssVars["--color-border-default"];
    cssVars["--color-neutral-muted"] =
      colors["button.secondaryBackground"] ||
      (isDark ? "rgba(255, 255, 255, 0.1)" : "rgba(0, 0, 0, 0.1)");

    cssVars["--color-accent-fg"] =
      colors["textLink.foreground"] ||
      colors["button.background"] ||
      colors["focusBorder"] ||
      "#007acc";
    cssVars["--color-accent-emphasis"] =
      colors["textLink.activeForeground"] ||
      colors["button.hoverBackground"] ||
      cssVars["--color-accent-fg"];

    cssVars["--color-btn-fg"] =
      colors["button.foreground"] || (isDark ? "#ffffff" : "#000000");
    cssVars["--color-btn-hover-bg"] =
      colors["button.hoverBackground"] || cssVars["--color-accent-emphasis"];
    cssVars["--color-tab-active-fg"] =
      colors["tab.activeForeground"] || cssVars["--color-btn-fg"];

    cssVars["--color-success-fg"] =
      colors["terminal.ansiGreen"] ||
      colors["gitDecoration.addedResourceForeground"] ||
      "#89d185";
    cssVars["--color-attention-fg"] =
      colors["terminal.ansiYellow"] ||
      colors["gitDecoration.modifiedResourceForeground"] ||
      "#cca700";
    cssVars["--color-danger-fg"] =
    ...
    return { name, isDark, cssVars, colors, rules };
  } catch (e) {
...
export function applyThemeConfig(config: ThemeConfig) {
  ...
  const rootStyles = Object.entries(config.cssVars)
    .map(([k, v]) => `${k}: ${v};`)
    .join("\n");
  styleTag.innerHTML = `:root[data-theme="vscode"] {\n${rootStyles}\n}`;
  document.documentElement.dataset.theme = "vscode";
  document.documentElement.dataset.themeType = config.isDark ? "dark" : "light";

  const bgHex = (config.cssVars["--color-canvas-default"] || "").replace("#", "");
  if (bgHex.length === 6) {
...
export function parseAndApplyVscodeTheme(themeJsonStr: string, name: string) {
  const config = parseVscodeTheme(themeJsonStr, name);
  if (config) {
