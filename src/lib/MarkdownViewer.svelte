...
import { t } from './utils/i18n.js';
	...
	onMount(() => {
		const handleGlobalClick = (event: MouseEvent) => {
			...
			if (target?.closest('.find-bar') || target?.closest('[data-find-toggle="true"]')) return;
			findOpen = false;
		};
	...
	async function selectFile() {
		const selected = await open({
			...
			filters: [
				...
				{ name: 'Código', extensions: ['js', 'jsx', 'ts', 'tsx', 'py', 'html', 'css', 'scss', 'less', 'java', 'c', 'h', 'cpp', 'hpp', 'cs', 'go', 'rs', 'php', 'rb', 'sh', 'bash', 'ps1', 'sql', 'xml', 'yaml', 'yml', 'toml'] },
				{ name: 'All Files', extensions: ['*'] },
			],
		});
					...
					<div 
						...
						<div class="viewer-content">
							<article
								...
								class="markdown-body full-width {settings.showToc ? 'toc-active' : ''}"
