...
export const tauriCommands = {
  ...
  copyFile: (src: string, dest: string) =>
    command<void>("copy_file", { src, dest }),
  ...
  listDirectory: (path: string) =>
    command<string[]>("list_directory_contents", { path }),
  ...
  renderMarkdown: (content: string) =>
    command<string>("render_markdown", { content }),
  ...
  readVscodeTheme: (name: string) =>
    command<string>("read_vscode_theme", { name }),
  fetchVscodeTheme: (url: string) =>
    command<string>("fetch_vscode_theme", { url }),
  ...
  installApp: (options: InstallOptions) =>
    command<void>("install_app", options),
