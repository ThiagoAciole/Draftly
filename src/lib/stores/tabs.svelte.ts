...
class TabManager {
		...
		try {
			...
			if (data && Array.isArray(data.tabs)) {
				this.tabs = data.tabs.map((tab: Tab) => {
					if (tab.path === '' && tab.newFileType && tab.newFileType !== 'markdown') {
						return {
							...tab,
							isSplit: false,
							isEditing: true,
					...
					return {
						...tab,
						isSplit: true,
						splitRatio: !tab.isSplit || tab.splitRatio === 0.5 ? 0.6 : tab.splitRatio,
					};
				});
	...
	addNewTab(type: NewFileType = 'markdown') {
		const id = crypto.randomUUID();
		const content = '';
		const extension = type === 'markdown' ? 'md' : type === 'json' ? 'json' : 'txt';
