...
describe("Markdown navigation", () => {
  ...
  it.each([
    ...
    "does not treat external or non-Markdown links as document navigation: %s",
    (href) => {
  ...
  it("resolves relative links from the current document", () => {
    expect(
      resolveMarkdownTarget("C:/notes/daily/today.md", "../guide.md"),
  ...
  it("compares Windows paths without case or separator differences", () => {
    ...
    ).toBe(normalizeComparableMarkdownPath("c:/notes/guide.md", "windows"));
  });
});
