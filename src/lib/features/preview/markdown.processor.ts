...
import {
  markdownAlertIcons,
  markdownChevronDownIcon,
  markdownCopyIcon,
  markdownHeaderChevronIcon,
} from "../../icons/markdown-html-icons.js";
...
export { isYoutubeLink, getYoutubeId };
  ...
  const walker = document.createTreeWalker(root, NodeFilter.SHOW_TEXT, {
    acceptNode(node) {
      ...
      while (curr && curr !== root) {
        if (curr.hasAttribute("data-math")) return NodeFilter.FILTER_REJECT;
...
function processTaskItems(root: Element) {
  for (const input of Array.from(
    ...
    if (
      inputParagraph?.tagName === "P" &&
      inputParagraph.parentElement === li
    ) {
    ...
    for (const n of afterInput) {
      ...
      if (
        n.nodeType === 1 &&
        taskTextBoundaryTags.has((n as Element).tagName)
      ) {
...
export function processHtmlPreview(html: string, filePath: string): string {
...
export function processMarkdownHtml(
  ...
  const stripLeadingBreaks = (node: Node) => {
    ...
    for (const br of Array.from(brs)) {
      ...
      while (prev) {
        if (
          prev.nodeType === 3 &&
          prev.textContent?.replace(/\xA0|\s|&nbsp;/g, "").trim()
        ) {
          isLeading = false;
          break;
    ...
    while (node.firstChild) {
      ...
      if (
        child.nodeType === 3 &&
        child.textContent?.replace(/\xA0|\s|&nbsp;/g, "").trim() === ""
      ) {
        child.parentElement?.removeChild(child);
      } else if (
        child.nodeType === 1 &&
        (child as Element).tagName === "P" &&
        (child as Element).innerHTML.replace(/\xA0|\s|&nbsp;/g, "").trim() ===
          ""
      ) {
        child.parentElement?.removeChild(child);
      } else {
  ...
  for (const bq of Array.from(doc.querySelectorAll("blockquote"))) {
    ...
    if (textNode && matchResult) {
      ...
      while (currentLineNode) {
        if (
          currentLineNode.nodeType === 1 &&
          (currentLineNode as Element).tagName === "BR"
        ) {
      ...
      const svgIconHtml = markdownAlertIcons[type] || "";
      if (svgIconHtml) {
      ...
      if (isFoldable) {
        ...
        chevron.innerHTML = markdownChevronDownIcon;
  ...
  for (const h of headings) {
    ...
    chevron.innerHTML = markdownHeaderChevronIcon;
...
export async function renderRichContent(
  ...
  for (const block of codeBlocks) {
    ...
    if (preEl && preEl.tagName === "PRE") {
      ...
      if (hasExplicitLang && langClass) {
        ...
        label.innerHTML = markdownCopyIcon;
        wrapper.appendChild(label);
      }
    }
  }
  ...
  if (katex) {
    ...
    for (const el of Array.from(mathElements)) {
      ...
      const mathSource =
        el.getAttribute("data-math-source") || el.textContent || "";
      try {
