	...
	function applyMarkdownToolbarAction(action: MarkdownToolbarAction) {
		...
		switch (action) {
			case 'heading': {
				...
				if (isEmpty && model.getLineContent(selection.startLineNumber).trim() === '') {
					placeholderOffset = {
						start: 4,
			...
			case 'quote':
				transformLines((line) => `> ${line.replace(/^\s*>\s?/, '') || 'texto'}`);
				if (isEmpty && model.getLineContent(selection.startLineNumber).trim() === '') {
...
<style>
	...
	:global(.monaco-editor .monaco-scrollable-element > .scrollbar.vertical > .slider) {
		left: 2px !important;
		width: 3px !important;
		border-radius: 999px;
	...
	:global(
		.monaco-editor .monaco-scrollable-element > .scrollbar.vertical > .slider.active
	) {
		background: color-mix(
			in srgb,
	...
	:global(.monaco-editor .monaco-scrollable-element > .scrollbar.horizontal > .slider) {
		top: 2px !important;
