  ...
  async function handleInstall() {
    ...
    try {
      ...
      if (
        ...
        (isInstalled ? installedAllUsers : allUsers)
      ) {
  ...
  <div class="content">
    <div class="header">
      ...
      {#if isInstalled}
        <div class="version-comparison">
          <span class="v-label">{t("installer.current")}</span>
          v{installedVersion}
          <span class="v-arrow">→</span>
          <span class="v-label">{t("installer.target")}</span>
          v{installerVersion}
        </div>
      {:else}
        <p class="subtitle">
          {t("installer.simpleMarkdownViewer")}
          <span class="v-lite">v{installerVersion}</span>
        </p>
      {/if}
    </div>
      ...
      <div class="setup-box">
        {#if !isInstalled}
          <div class="scope-toggle">
            <button class:active={!allUsers} onclick={() => (allUsers = false)}
              >{t("installer.justMe")}</button
            >
            <button class:active={allUsers} onclick={() => (allUsers = true)}
              >{t("installer.allUsers")}</button
            >
          </div>
        {/if}
        ...
        <div class="options-container">
          ...
          {:else}
            <div class="maintenance-options">
              <p class="status-msg">
                {t("installer.installedFor")}
                <strong
                  >{installedAllUsers
                    ? t("installer.allUsers")
                    : "Current User"}</strong
                >
              </p>
              <div class="options">
        ...
        <div class="actions">
          {#if isInstalled}
            ...
            <button class="install-btn" onclick={handleInstall}
              >{t("installer.updateRepair")}</button
            >
          {:else}
            <button class="install-btn" onclick={handleInstall}>
              {t(
                allUsers
                  ? "installer.installForAllUsers"
    ...
    {:else}
      <div class="installing-state">
        <div class="spinner"></div>
        <p>
          {t(isInstalled ? "installer.updating" : "installer.installing")}
          {t("installer.draftly")}
        </p>
      </div>
    {/if}
