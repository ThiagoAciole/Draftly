...
/// Draftly targets Tauri v2 (Rust 1.70+), so we can rely on this everywhere.
///
...
fn atomic_write(target: &Path, bytes: &[u8]) -> std::io::Result<()> {
    ...
    let file_name = target
        ...
        .unwrap_or_else(|| "draftly".to_string());
    let nanos = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .map(|d| d.as_nanos())
        .unwrap_or(0);
    let pid = std::process::id();
    let temp_name = format!(".{}.draftly-tmp-{}-{}", file_name, pid, nanos);
...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .on_menu_event(|app, event| {
            ...
            // which would broadcast to every webview. Draftly is currently
