...
#[cfg(target_os = "windows")]
const FILE_TYPE_ICONS: [(&str, &[u8]); 5] = [
    (
        "markdown.ico",
        include_bytes!("../icons/file-types/markdown.ico"),
    ),
    ("txt.ico", include_bytes!("../icons/file-types/txt.ico")),
    ("js.ico", include_bytes!("../icons/file-types/js.ico")),
    ("html.ico", include_bytes!("../icons/file-types/html.ico")),
    ("json.ico", include_bytes!("../icons/file-types/json.ico")),
];

#[cfg(target_os = "windows")]
const FILE_ASSOCIATIONS: [(&[&str], &str, &str, &str); 5] = [
    (
    ...
    (&["txt"], "Draftly.Text", "Text File", "txt.ico"),
    (
        &["js", "jsx"],
        "Draftly.JavaScript",
        "JavaScript File",
    ...
    (&["html", "htm"], "Draftly.HTML", "HTML File", "html.ico"),
    (&["json"], "Draftly.JSON", "JSON File", "json.ico"),
];
...
pub async fn install_app(
    ...
    write_file_type_icons(&install_dir).map_err(|e| e.to_string())?;
    ...
    key.set_value("DisplayIcon", &target_exe.to_str().unwrap())
        .map_err(|e| e.to_string())?;
    key.set_value("Publisher", &"ThiagoAciole")
        .map_err(|e| e.to_string())?;
...
pub async fn uninstall_app(
    ...
    for root_h in [HKEY_CURRENT_USER, HKEY_LOCAL_MACHINE] {
        ...
        for (extensions, prog_id, _, _) in FILE_ASSOCIATIONS {
            for extension in extensions {
                unregister_file_association(&root, extension, prog_id);
            }
            let _ = root.delete_subkey_all(format!("Software\\Classes\\{}", prog_id));
        }
        let _ = root.delete_subkey_all("Software\\Classes\\Draftly.File");
    }
...
fn register_file_association(exe_path: &Path, all_users: bool) -> Result<(), std::io::Error> {
    ...
    let icon_dir = install_dir.join("file-icons");

    for (extensions, prog_id, description, icon_name) in FILE_ASSOCIATIONS {
        for extension in extensions {
            let (extension_key, _) =
                root.create_subkey(format!("Software\\Classes\\.{}", extension))?;
            extension_key.set_value("", &prog_id)?;
        }

        let (file_key, _) = root.create_subkey(format!("Software\\Classes\\{}", prog_id))?;
        file_key.set_value("", &description)?;

        let (icon_key, _) = file_key.create_subkey("DefaultIcon")?;
        icon_key.set_value("", &format!("\"{}\",0", icon_dir.join(icon_name).display()))?;
...
#[cfg(target_os = "windows")]
fn write_file_type_icons(install_dir: &Path) -> Result<(), std::io::Error> {
    let icon_dir = install_dir.join("file-icons");
    fs::create_dir_all(&icon_dir)?;

    for (file_name, bytes) in FILE_TYPE_ICONS {
        fs::write(icon_dir.join(file_name), bytes)?;
    }
...
#[cfg(target_os = "windows")]
fn unregister_file_association(root: &RegKey, extension: &str, prog_id: &str) {
    let extension_path = format!("Software\\Classes\\.{}", extension);
    if let Ok(key) = root.open_subkey_with_flags(&extension_path, KEY_READ | KEY_WRITE) {
        if key
            .get_value::<String, _>("")
            .is_ok_and(|current| current == prog_id)
        {
